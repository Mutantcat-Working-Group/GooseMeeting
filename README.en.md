...
> Production security hardening and multi-device integration testing are still required. Read [7. Security and Limitations](#7-security-and-limitations) before deploying. Frontend management controls are not a substitute for server-side authorization.
...
- Create or join password-protected rooms.
- Camera and microphone calls, screen sharing, and media source switching.
- Participant previews and enlarged video.
- In-meeting text chat and notification messages.
- Host controls: chat restrictions, microphone muting, video disabling, and participant removal.
...
Public versions use `major.minor.YYYYMMDD`. Windows numeric resource fields are limited to 65535 per part, so packaging maps the version to `1.0.2026+920` (native four-part `1.0.2026.920`); the UI, tag, release and download filenames retain `1.0.20260920`.

#### Automated Releases

Pushing a `v*` version tag runs the four native builds in [`.github/workflows/release.yml`](.github/workflows/release.yml). Only after every build succeeds are all four installers and the SHA-256 checksum list uploaded and the Release published. A failed build does not publish an incomplete new release. The same workflow can be dispatched manually with an existing version tag, and enabling `preflight` produces Actions artifacts only, without creating a Release.

Before tagging, update the application version in `package.json`, `package-lock.json`, `src-tauri/Cargo.toml` and `src-tauri/Cargo.lock`, commit, then push a matching version tag:
...
The workflow uses only the repository's built-in `GITHUB_TOKEN`, and the publishing job needs `contents: write`. No Apple certificate or paid signing secret is needed for the default build.
